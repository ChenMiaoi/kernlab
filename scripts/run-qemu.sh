...
Environment variables:
  ...
  QEMU_BIN         QEMU binary (default: arch-specific)
...
case "${ARCH}" in
    ...
    arm|arm32)
        ...
        QEMU_BIN_DEFAULT="qemu-system-arm"
    ...
    riscv|riscv64)
        ...
        QEMU_BIN_DEFAULT="qemu-system-riscv64"
        QEMU_MACHINE_DEFAULT="virt"
...
QEMU_BIN="${QEMU_BIN:-$(command -v "${QEMU_BIN_DEFAULT}" || true)}"
...
if [[ -z "${QEMU_BIN}" || ! -x "${QEMU_BIN}" ]]; then
    echo "QEMU binary not found for ARCH=${ARCH} (expected ${QEMU_BIN_DEFAULT})." >&2
    exit 1
fi
